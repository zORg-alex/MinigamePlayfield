...
public class UIArcBar : MaskableGraphic
{
    ...
    public float innerRadiusCoef { get => (OuterRadius - InnerThickness) / OuterRadius; }
    public float rightmostX { get => ArcAngle < 180 ? Mathf.Sin(Mathf.Deg2Rad * (-startAngle)) * OuterRadius : OuterRadius; }
    public float bottommostY { get => Mathf.Cos(Mathf.Deg2Rad * startAngle) * (ArcAngle < 180 ? OuterRadius - InnerThickness : OuterRadius); }
    public float arcFullHeight { get => OuterRadius - bottommostY; }
    public float arcFullWidth { get => 2 * rightmostX; }
    ...
    private void SetTransformStuff()
    {
